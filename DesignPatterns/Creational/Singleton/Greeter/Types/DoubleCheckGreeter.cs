...
public class DoubleCheckGreeter : BaseGreeter
{
    ...
    private static DoubleCheckGreeter? instance;
    ...
    [SuppressMessage("Maintainability", "CA1508:Avoid dead conditional code",
        Justification = "If the first check determines that instance is null, " +
                        "that might not be the case for the second check in multi-threaded scenarios.")]
        ...
        get
        {
            if (instance == null)
            {
                lock (Padlock)
                {
                    if (instance == null)
                    {
                        instance = new DoubleCheckGreeter();
                    }
                }
            }
